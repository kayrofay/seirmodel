...
simfunc <- function(A = 1786, tao_arrival = 14, tao_1=6, tao_2=10,mu=0.66, y=1, C_max=13, i_N=0.037,
  ...
  for (i in 1:len){
    ...
    record[i,'7 day rolling average'] = mean(record[max((i-14),1) :i,3], na.rm=TRUE)
    
    
    # W_N: factors that affect contact rate. Students tend to change behavior according to current covid cases
    record[i, 'factors that affect contact rate'] = exp(-h * ((record[i,'7 day rolling average'])/N))
    record[i, 'C'] = C_max * record[i,'factors that affect contact rate']
...
server <- function(input, output) {
  ...
  record_reac <- reactive({ 
    simfunc(A = input$students,C_max = input$cmax, i_N =input$i_n, 
            phi=as.numeric(input$phi), h=input$h)
  })
  cum_reac <- reactive({ 
