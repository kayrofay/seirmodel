#this one has one plot from plotly
...
server <- function(input, output) {
 ...
 C_max <- reactive({input$cmax})
 
 i_N <- reactive({input$i_n})
 ...
 record_reac <- eventReactive(c(input$students, input$cmax, input$i_n), {
    record = data.frame(matrix(0,nrow=len, ncol=4))
  ...
  output$distPlot <- renderPlotly({
    ggplot(data=record_reac()) +
      geom_line(aes(x=days, y=X1, color="Susceptible")) +
      geom_line(aes(x=days, y=X2, color="Exposed"))+
      geom_line(aes(x=days, y=X3, color="Infected"))+
      geom_line(aes(x=days, y=X4, color="Recovered"))+
