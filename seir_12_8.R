...
#this one has four different plots
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    # Show a plot of susceptible, exposed, infected, recovered
    mainPanel(
      fluidRow(
        plotOutput("plots")
      )
...
# plots output
  output$plots <- renderPlot({
    ...
    p1<- ggplot(data=record_reac()) +
      geom_line(aes(x=1:len, y=X1)) + 
      labs(y="Cumulative cases", x= "Days") +
      ggtitle("Susceptible")
    
    p2 <- ggplot(data=record_reac()) +
      geom_line(aes(x=1:len, y=X2), color="orange") + 
      labs(y="Cumulative cases", x= "Days") +
      ggtitle("Exposed")
    
    p3 <- ggplot(data=record_reac()) +
      geom_line(aes(x=1:len, y=X3), color="red") + 
      labs(y="Cumulative cases", x= "Days") +
      ggtitle("Infected")
    
    p4 <- ggplot(data=record_reac()) +
